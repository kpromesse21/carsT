<x-app-layout>
    ...
    <div class="card">
        ...
        @empty
            ...
            <x-text-input class="block mt-1  w-full" type="email" name="mail" required /> --}}
                ...
                <select name="categorie" default="C" value="{{ $car->categorie }}" id="" class="w-full rounded">
    ...
    <div class="card">
        ...
        <form action="{{ route('car.recherche') }}" method="post" class="search-form"> @csrf<input
                name="search" /><input type="hidden" name="id" value="{{ $car->id }}">
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="{{route('car.proprio')}}" method="post">
                                @csrf <input type="hidden" name="user" value="{{ $item->id }}"> <input type="hidden" name="car" value="{{$car->id}}"> <input type="submit" value="affecter">
                            </form>
                        </td>
                    </tr>
                @endforeach
